\newpage 
\subsection{IU 5.1.1.1.1 Registrar paso}

\subsubsection{Objetivo}
	
	Esta pantalla permite al actor registrar la información de un paso.

\subsubsection{Diseño}

    En la figura ~\ref{IU 5.1.1.1.1} se muestra la pantalla ``Registrar paso'' que permite registrar un paso. El actor deberá ingresar la información solicitada, 
    esto incluye indicar quién realiza la acción, el verbo y la redacción del paso.\\
    
    Cuando el actor requiera un verbo que no está en la lista podrá seleccionar la opción ``Otro'' y el sistema mostrará un campo para 
    que especifique el verbo que requiera como se muestra en la figura ~\ref{IU 5.1.1.1.1b}. \\
    
    
    Una vez ingresada la información solicitada en la pantalla, el actor deberá oprimir el botón 
    \cdtButton{Aceptar}, el sistema validará y agregará el paso a la trayectoria solo si se han cumplido todas las reglas de negocio establecidas.  \\


    \IUfig[.9]{cu5.1.1.1.1/images/iu.png}{IU 5.1.1.1.1}{Registrar paso}
    \IUfig[.9]{cu5.1.1.1.1/images/iu2.png}{IU 5.1.1.1.1b}{Registrar paso: Otro verbo}


\subsubsection{Comandos}
\begin{itemize}
	\item \cdtButton{Aceptar}: Permite al actor guardar el registro del paso, dirige a la pantalla \cdtIdRef{IU 5.1.1.1}{Registrar trayectoria} o a la pantalla \cdtIdRef{IU 5.1.1.2}{Modificar trayectoria}, según corresponda.
	\item \cdtButton{Cancelar}: Permite al actor cancelar el registro del paso, dirige a la pantalla \cdtIdRef{IU 5.1.1.1}{Registrar trayectoria} o a la pantalla \cdtIdRef{IU 5.1.1.2}{Modificar trayectoria}, según corresponda.
\end{itemize}

\subsubsection{Mensajes}

	
\begin{description}
	\item[ \cdtIdRef{MSG4}{Dato obligatorio}:] Se muestra en la pantalla \cdtIdRef{IU 5.1.1.1.1}{Registrar paso} cuando no se ha ingresado un dato marcado como obligatorio.
	\item[ \cdtIdRef{MSG6}{Longitud inválida}:] Se muestra en la pantalla \cdtIdRef{IU 5.1.1.1.1}{Registrar paso} cuando se ha excedido la longitud de alguno de los campos.
\end{description}
